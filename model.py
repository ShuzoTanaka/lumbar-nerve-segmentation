...
class MultiClassModel(L.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # オリジナルサイズを取得
        original_shape = images.shape[2:]  # (H, W, D) の形状を取得

        # 予測された mask をオリジナルサイズにリサイズ
        ...
        masks = masks.permute(
            0, 4, 1, 2, 3
        ).float()  # [batch_size, depth, height, width, num_classes] -> [batch_size, num_classes, depth, height, width]

        # バイナリマスクに変換（閾値0.5）し、適切な型にキャスト
        binary_predictions = (
            (predictions.sigmoid() > 0.5).cpu().numpy().astype(np.uint8)
        )
        ...
        for i in range(predictions.shape[0]):  # バッチ内の各サンプル
            sample_idx = self.global_sample_idx  # 連続した番号にする
            self.global_sample_idx += 1  # 次のサンプルのためにカウンタを増やす
