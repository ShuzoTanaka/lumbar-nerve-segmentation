...
| クラス | Dice係数 |
|---|---|
| **神経根 nerve (class 1)** | **0.7399** |
| 硬膜管 dural sac (class 2) | 0.5548 |
| nerve + dural sac 平均 | 0.6473 |

採用理由は次の「2D vs 3D 比較実験」を参照。

## 2D vs 3D 比較実験

同一の34症例・同一のホールドアウト5症例でテストした結果。**Diceは両モデルとも「症例(ボリューム)全体を1つの混同行列として集計し、症例ごとに算出したDiceを5症例で平均する」方法に統一している**（2Dは元々スライスごとにDiceを算出して平均しており、3Dの症例単位集計と方法が異なっていたため、`test_2D_v2.py`を修正して揃えた）：

| モデル | nerve Dice | dural sac Dice | 平均 |
|---|---|---|---|
| **2D U-Net（採用）** | **0.7399** | 0.5548 | 0.6473 |
...
- ホールドアウト症例のうち1例（00008）はGTラベルに硬膜管クラスの画素が1つも存在せず、この症例の硬膜管Diceは強制的に0になる。5症例平均のdural sac Diceはこの1例の影響を強く受けているため、参考値として扱うこと
